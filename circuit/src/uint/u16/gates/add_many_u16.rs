...
/// A gate to perform addition on `num_addends` different 16-bit values, plus a small carry
#[derive(Copy, Clone, Debug, Default)]
pub struct U16AddManyGate<F: RichField + Extendable<D>, const D: usize> {
    pub num_addends: usize,
    pub num_ops: usize,
    _phantom: PhantomData<F>,
}

impl<F: RichField + Extendable<D>, const D: usize> U16AddManyGate<F, D> {
    ...
    pub fn num_result_limbs() -> usize {
        ceil_div_usize(16, Self::limb_bits())
    }
...
impl<F: RichField + Extendable<D>, const D: usize> Gate<F, D> for U16AddManyGate<F, D> {
    ...
    fn eval_unfiltered(&self, vars: EvaluationVars<F, D>) -> Vec<F::Extension> {
        ...
        for i in 0..self.num_ops {
            ...
            // Check that output_carry * base + output_result = addend_0 + addend_1 + ... + addend_n + carry
            // Result isn't overflow goldilocks field because output_carry is max 2**4 - 1
            ...
            // Check that output_result is in range 4 ** Self::num_result_limbs() = 2 ** 16
            constraints.push(combined_result_limbs - output_result);
            // Check that output_carry is in range 4 ** Self::num_carry_limbs() = 2 ** 4
...
pub struct U16AddManyGenerator<F: RichField + Extendable<D>, const D: usize> {
    gate: U16AddManyGate<F, D>,
    row: usize,
    i: usize,
    _phantom: PhantomData<F>,
}
...
impl<F: RichField + Extendable<D>, const D: usize> SimpleGenerator<F, D>
    for U16AddManyGenerator<F, D>
{
    fn id(&self) -> String {
        "U16AddManyGenerator".to_string()
    }
    ...
    fn deserialize(src: &mut Buffer, common_data: &CommonCircuitData<F, D>) -> IoResult<Self> {
        let gate = U16AddManyGate::deserialize(src, common_data)?;
    ...
    fn run_once(
        ...
        let output_result_u64 = output_u64 & ((1 << 16) - 1);
