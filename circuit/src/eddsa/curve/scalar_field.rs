...
impl Add<ECgFp5Scalar> for ECgFp5Scalar {
    type Output = ECgFp5Scalar;

    fn add(self, other: ECgFp5Scalar) -> ECgFp5Scalar {
        ECgFp5Scalar::add(self, other)
    }
}

impl AddAssign<ECgFp5Scalar> for ECgFp5Scalar {
    fn add_assign(&mut self, other: ECgFp5Scalar) {
        *self = ECgFp5Scalar::add(*self, other)
    }
}

impl Sub<ECgFp5Scalar> for ECgFp5Scalar {
    type Output = ECgFp5Scalar;

    fn sub(self, other: ECgFp5Scalar) -> ECgFp5Scalar {
        ECgFp5Scalar::sub(self, other)
    }
}

impl SubAssign<ECgFp5Scalar> for ECgFp5Scalar {
    fn sub_assign(&mut self, other: ECgFp5Scalar) {
        *self = ECgFp5Scalar::sub(*self, other)
    }
}
...
impl Neg for ECgFp5Scalar {
    ...
    fn neg(self) -> ECgFp5Scalar {
        ECgFp5Scalar::neg(self)
    }
}

impl Mul<ECgFp5Scalar> for ECgFp5Scalar {
    type Output = ECgFp5Scalar;

    fn mul(self, other: ECgFp5Scalar) -> ECgFp5Scalar {
        ECgFp5Scalar::mul(self, other)
    }
}

impl MulAssign<ECgFp5Scalar> for ECgFp5Scalar {
    fn mul_assign(&mut self, other: ECgFp5Scalar) {
        *self = ECgFp5Scalar::mul(*self, other)
    }
}

impl Div<ECgFp5Scalar> for ECgFp5Scalar {
    type Output = ECgFp5Scalar;

    fn div(self, other: ECgFp5Scalar) -> ECgFp5Scalar {
        self * other.inverse()
    }
}

impl DivAssign<ECgFp5Scalar> for ECgFp5Scalar {
    #[allow(clippy::suspicious_op_assign_impl)]
    fn div_assign(&mut self, other: ECgFp5Scalar) {
        *self *= other.inverse()
    }
}

impl Sum for ECgFp5Scalar {
    fn sum<I: Iterator<Item = ECgFp5Scalar>>(iter: I) -> ECgFp5Scalar {
        iter.fold(ECgFp5Scalar::ZERO, |acc, x| acc + x)
    }
}

impl Product<ECgFp5Scalar> for ECgFp5Scalar {
    fn product<I: Iterator<Item = ECgFp5Scalar>>(iter: I) -> ECgFp5Scalar {
        iter.fold(ECgFp5Scalar::ONE, |acc, x| acc * x)
    }
}
...
impl ECgFp5Scalar {
    ...
    */
    ...
    pub fn from_gfp5(x: QuinticExtension<F>) -> Self {
...
impl Signed161 {
    fn from_scalar(s: ECgFp5Scalar) -> Self {
        Self([s.0[0], s.0[1], s.0[2]])
    }

    /// Convert that value into a scalar (integer modulo n).
    pub fn to_scalar_vartime(self) -> ECgFp5Scalar {
        ...
        if neg {
            ...
            -ECgFp5Scalar([tmp[0], tmp[1], tmp[2], 0, 0])
        } else {
            ECgFp5Scalar([tmp[0], tmp[1], tmp[2], 0, 0])
        }
    }
    ...
    pub(crate) fn recode_signed_5(self) -> [i32; 33] {
        ...
        let mut ss = [0i32; 33];
        ECgFp5Scalar::recode_signed_from_limbs(&tmp, &mut ss, 5);
        ss[32] -= 1;
        ss
    }
...
impl Signed640 {
    ...
    fn from_mul_scalars(a: ECgFp5Scalar, b: ECgFp5Scalar) -> Self {
...
mod tests {
    ...
    fn test_scalar_ops() {
        ...
        for i in 0..51 {
            let (s1, c1) = ECgFp5Scalar::try_from_noncanonical_bytes(&buf1[..i]);
            let (s2, c2) = ECgFp5Scalar::try_from_noncanonical_bytes(&buf2[..i]);
            let (s3, c3) = ECgFp5Scalar::try_from_noncanonical_bytes(&buf3[..i]);
        ...
        let s4 = ECgFp5Scalar::from_noncanonical_bytes(&buf4[..]);
        assert!(s4.encode() == buf5);
        let (s5, c5) = ECgFp5Scalar::try_from_noncanonical_bytes(&buf5[..]);
    ...
    test_field_arithmetic!(crate::eddsa::curve::scalar_field::ECgFp5Scalar);
}
