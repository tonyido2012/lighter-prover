...
use crate::ecdsa::curve::curve_types::{AffinePoint, Curve, CurveScalar};
...
pub trait CircuitBuilderCurve<F: RichField + Extendable<D>, const D: usize> {
    ...
    fn conditional_curve_assert_valid<C: Curve>(
        &mut self,
        flag: BoolTarget,
    ...
    fn conditional_curve_add<C: Curve>(
        &mut self,
        cond: BoolTarget,
        p1: &AffinePointTarget<C>,
        p2: &AffinePointTarget<C>,
...
impl<F: RichField + Extendable<D>, const D: usize> CircuitBuilderCurve<F, D> for Builder<F, D> {
    fn constant_affine_point<C: Curve>(&mut self, point: AffinePoint<C>) -> AffinePointTarget<C> {
    ...
    fn conditional_curve_assert_valid<C: Curve>(
        ...
        self.conditional_connect_nonnative(flag, &y_squared, &rhs);
    }
    ...
    fn curve_double<C: Curve>(&mut self, p: &AffinePointTarget<C>) -> AffinePointTarget<C> {
        ...
        let inv_double_y = self.inv_nonnative(&double_y, None);
    ...
    fn curve_add<C: Curve>(
        ...
        let v_inv = self.inv_nonnative(&v, None);
    ...
    fn conditional_curve_add<C: Curve>(
        ...
        let u = self.sub_nonnative(y2, y1);
        let v = self.sub_nonnative(x2, x1);
        let v_inv = self.conditional_inv_nonnative(cond, &v, None);
        let s = self.mul_nonnative(&u, &v_inv);
        let s_squared = self.mul_nonnative(&s, &s);
        let x_sum = self.add_nonnative(x2, x1);
        let x3 = self.sub_nonnative(&s_squared, &x_sum);
...
mod tests {
    ...
    use crate::ecdsa::gadgets::curve::CircuitBuilderCurve;
    ...
    #[test]
    fn test_curve_mul_bool_add() -> Result<()> {
        const D: usize = 2;
        type C = PoseidonGoldilocksConfig;
        type F = <C as GenericConfig<D>>::F;
        ...
        let g_plus_2g_actual = builder.curve_mul_bool_add(&g_expected, &double_g_target, t);
        let g_actual = builder.curve_mul_bool_add(&g_expected, &double_g_target, f);
